...
SEC_HEADERS = {
    "User-Agent": "Givaudan Peer Dashboard lyubomir.dachev@example.com",
...
def annual_item_year(item: dict[str, Any]) -> int | None:
    frame = str(item.get("frame", ""))
    if frame:
        if len(frame) == 6 and frame.startswith("CY") and frame[2:].isdigit():
            return int(frame[2:])
        return None
        ...
        for unit_values in fact.get("units", {}).values():
            for item in unit_values:
                if item.get("form") != "10-K" or item.get("fp") != "FY":
                    continue
                val = item.get("val")
                fy_int = annual_item_year(item)
                if fy_int is None or val is None:
                    continue
                try:
                    val_float = float(val)
                except (TypeError, ValueError):
    ...
    for tag in tags:
        ...
        for unit_values in fact.get("units", {}).values():
            for item in unit_values:
                ...
                filed = item.get("filed", "")
                fy_int = annual_item_year(item)
                if fy_int is None or not filed:
                    continue
...
def analyst_summary(company: Company, metrics: dict[str, float | int | None]) -> str:
    ...
    return (
        ...
        f"Its biggest strength is its role as a {company.why.lower()}, which helps frame Givaudan's position across ingredients, fragrance, and beauty demand. "
        f"The key risk is whether profitability, leverage at {de:.2f}x D/E, and operating intensity at {opex:.1f}% OpEx can stay balanced through consumer and input-cost cycles. "
        "In industry context, the peer set separates companies with durable formulation or brand advantages from companies that rely more heavily on cyclical beauty spending."
    )
...
def main() -> None:
    ...
    payload = {
        "metadata": {
            "title": "Givaudan Peer Competitive Intelligence Dashboard",
