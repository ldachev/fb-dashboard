...
function renderMarketSummary(companies) {
  const strongest = maxBy(companies, "net_margin_pct");
  ...
  document.getElementById("market-summary").textContent =
    `This peer set shows how the fragrance and beauty market stretches from specialty ingredient suppliers to prestige brands, licensed fragrance houses, and consumer-facing retailers. Ingredient suppliers compete on formulation expertise, customer relationships, and operating discipline, while beauty and fragrance brands reveal how demand is shifting at the consumer end of the value chain. The strongest profiles pair resilient sales with healthy margins and manageable leverage, while weaker profiles depend more heavily on turnaround execution, brand recovery, or consumer-cycle improvement. Overall, the market favors companies that can translate innovation, scent-led demand, and brand relevance into consistent profits rather than growth alone.`;
  document.getElementById("summary-evidence").innerHTML = [
    evidenceItem("Strongest margin", strongest, `${fmtPct(metrics(strongest).net_margin_pct)} net margin, ${fmtPct(metrics(strongest).roa_pct)} ROA, ${fmtRatio(metrics(strongest).debt_to_equity)} D/E`),
    evidenceItem("Fastest grower", fastest, `${fmtPct(metrics(fastest).revenue_growth_pct)} revenue growth with ${fmtPct(metrics(fastest).net_margin_pct)} net margin`),
    evidenceItem("Largest revenue base", largest, `${fmtMoney(metrics(largest).revenue_m)} annual revenue`),
    evidenceItem("Highest leverage", mostLevered, `${fmtRatio(metrics(mostLevered).debt_to_equity)} D/E`),
    `<details><summary>Sector divide evidence</summary><p>Ingredient peers average ${fmtPct(ingredientMargin)} net margin. Beauty and fragrance-facing peers average ${fmtPct(beautyMargin)} net margin.</p></details>`,
  ].join("");
}
  ...
  return `
    <div class="legend">
      ${Object.entries(sectorColors).map(([sector, color]) =>
        `<span title="${sector} peer"><i style="background:${color}"></i>${sector}</span>`
      ).join("")}
    </div>`;
}
...
function strategicLabel(company) {
  const m = metrics(company);
  if (m.net_margin_pct > 12 && m.debt_to_equity < 1) return "Efficient Margin Leader";
  if (m.revenue_growth_pct > 15 && m.net_margin_pct > 0) return "Growth Compounder";
  if (m.net_margin_pct > 0 && m.revenue_growth_pct < 8) return "Mature Cash Generator";
...
function renderRadar(companies, selected) {
  ...
  document.getElementById("radar-chart").innerHTML = `
    ...
    <div class="radar-legend" aria-label="Radar chart legend">
      <span><i class="radar-key selected-key"></i>${selected.ticker}</span>
      <span><i class="radar-key peer-key"></i>Peer average</span>
    </div>`;
}
